...
function OnServerTenders(resultStr) {
...
function OnServerOffers(resultStr)
{
    //present them as a table
    var objWithPropertiesOnly = JSON.parse(resultStr);
...
function main() {
    ...
    /*//print the tenders
    {
        ...
        xmlhttp.onreadystatechange = function () { if (xmlhttp.readyState == 4 && xmlhttp.status == 200) OnServerTenders(xmlhttp.responseText); }
    ...
    */
    ...
    xmlhttp.onreadystatechange = function () { if (xmlhttp.readyState == 4 && xmlhttp.status == 200) OnServerOffers(xmlhttp.responseText); }
    xmlhttp.open("POST", "OfferForInstitute.php", true);
    xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlhttp.send("userData=" + user.toJSon());
