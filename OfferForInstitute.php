...
class ProductOffer
{
    ...
    function fromDb($conn, $user, $saverID)
    {
        //get the product's insititute
        $sql_searchProducts = "select * from products where ProductID = " . $this->ID . " and UserID = " . $saverID;
        ...
        //get my alternative offer for this product id
        $sql_alternatives = "select * from offers where userId = " . $saverID . " and ProductID = " . $this->ID . " and InstituteID = ". $user->institute->ID;
        ...
        while($row = mysqli_fetch_array($result))
        {
            ...
            $this->offer = new Offer($institute, $managementFee, $offerStatus, $submittion);
        }
        return TRUE;
...
class ProductTypeOffer
{
    ...
    function fromDb($conn, $user)
    {
        ...
        //get all products offers for this product type, for this institute
        $sql_searchProducts = 
            "select ProductID,UserID from offers where InstituteID = " . $user->institute->ID . " and ProductID in (select distinct ProductID from products where ProductTypeID = " . $this->ID . ")";
        $result = mysqli_query($conn, $sql_searchProducts);
        while($row = mysqli_fetch_array($result))
        {
            ...
            $productOffer->fromDb($conn, $user, $saverID);
